...
class PaymentController:
    ...
    def __call__(self):
        match self.payment_service:
            case PaymentService.MTN_RWANDA:
                self.validate_input_data(MomoPaymentSchema)
                return self.momo_payment()
            case PaymentService.AIRTEL_RWANDA:
                self.validate_input_data(AirtelPaymentSchema)
                return self.airtel_payment()
    ...
    def momo_payment(self):
        data = MomoPaymentSchema(**self.input_data.model_dump())
        push_payment = requests.post(
            url=PaymentPushCallback.MTN_PUSH,
            json=data.model_dump(),
        )
        return push_payment.json()

    def airtel_payment(self):
        data = AirtelPaymentSchema(**self.input_data.model_dump())
        push_payment = requests.post(
            url=PaymentPushCallback.AIRTEL_PUSH,
            json=data.model_dump(),
        )
        return push_payment.json()
...
class NetworkRegexSchema(Enum):
    mtn_rwanda = ["^250(78|79)[0-9]{7}$"]
    airtel_rwanda = ["^250(72|73)[0-9]{7}$"]
